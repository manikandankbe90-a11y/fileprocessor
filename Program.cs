 foreach (var file in csFiles)
 {
     try
     {
         ...
         if (versionConstMatches.Count > 0)
         {
             ...
             var newVersionConst = $"private const string Version{newVersion} = \"{newVersion}\";";

             // Check if new version already exists
             if (!content.Contains($"Version{newVersion}"))
             {
                 ...
                 content = Regex.Replace(content, latestVersionPattern, "${1}Version" + newVersion + "${2}");

                 modified = true;
                 Console.WriteLine($"  [OK] {Path.GetFileName(file)} - Added Version{newVersion} = \"{newVersion}\" and updated LatestVersion");
             }
         }
         ...
         if (upgradeDbMatches.Count > 0)
         {
             ...
             if (!content.Contains($"name=\"{newEntryName}\""))
             {
                 ...
                 if (lastDataMatch.Success)
                 {
                     ...
                     // Insert new entry after the existing one
                     var insertPos = content.IndexOf(existingEntry) + existingEntry.Length;
                     content = content.Insert(insertPos, lineEnding + "  " + newDataEntry);
 ...
 foreach (var batFile in batFiles)
 {
     try
     {
         ...
         // Find the last ECHO + sqlcmd pair and add new pair after the sqlcmd line
         // Pattern for sqlcmd line with UpgradeDBSVerXXtoYY.sql
         var sqlcmdPattern = @"sqlcmd[^\r\n]*UpgradeDBSVer(\d+)to(\d+)\.sql[^\r\n]*";
         var sqlcmdMatches = Regex.Matches(content, sqlcmdPattern, RegexOptions.IgnoreCase);

         if (sqlcmdMatches.Count > 0)
         {
             // Get the last UpgradeDBSVerXtoY numbers
             int lastFrom = 0;
             int lastTo = 0;
             ...
             // Check if new version already exists
             if (!content.Contains($"UpgradeDBSVer{lastTo}to{lastTo + 1}.sql"))
             {
                 // Find the corresponding ECHO line for this sqlcmd to get script number
                 ...
                 foreach (Match m in echoMatches)
                 {
                     ...
                     if (dbVersion > lastScriptNum || dbVersion == lastTo)
                     {
                 ...
                 var newSqlcmdLine = lastSqlcmdLine.Replace($"UpgradeDBSVer{lastFrom}to{lastTo}", $"UpgradeDBSVer{lastTo}to{lastTo + 1}");
                 ...
                 // Add new ECHO + sqlcmd pair after the last sqlcmd line
